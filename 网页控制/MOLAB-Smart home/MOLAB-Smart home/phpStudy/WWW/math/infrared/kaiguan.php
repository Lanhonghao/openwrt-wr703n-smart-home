	...
	if(http_request.readyState == 4){
		if(http_request.status == 0 || http_request.status == 200){
			...
			if(str.length>2){
				...
				if( str[3]=="2" ){
					document.getElementById("device_status").innerHTML="<label style=\"color:#FF0000\">异常-(检查红外线/sid和nid)</label>";
				}
...
<table class="top" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        ...
        <td>智能创客－红外线</td>
        <td><a href="../infrared/index.php"><button>列表</button></a></td>
    </tr>
</table>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
		 ...
		 <tr>
            <td width="50%" align="center" >
                <input maxlength="32" type="text" class="search_text" id="rdata" name="rdata" value="<?php echo $data;?>"  width="50%"/>
				<BR><label style="color:#00CC00; font-size:9px;">可以直接填写编码格式，如1,16754775,32,（类型,值,字节）</label>
            </td>
        </tr>	
		<tr>
            <td width="100%" align="center" >
				   ...
				   <?php 
					...
					if( $data=="0" )
					{
						echo '<a onclick="onoff('.$sid.','.$nid.')"> <img src="../../img/index41hwxon_.png" id="imgonoff" /> </a>';						
					}
					else 
					{						
						echo '<a onclick="onoff('.$sid.','.$nid.')"> <img src="../../img/index41hwxon.png" id="imgonoff" /> </a>';		
					}
		...
		<tr>
            <td width="100%" align="center" >				
				...
				<a href="../../../devicetime.php?mode=&type=5&sid=<?php echo $sid?>&nid=<?php echo $nid?>" style="color:#33CC33;">				                  
				   <?php 				  
		...
		<tr>
			...
			<?php 
				...
				while($row = mysql_fetch_array($result)) 
				{
					...
					$time=$row['time'];
					*/
					echo "红外线：".sprintf("%02d",$row[1])."时间：".getTimeDifference($row[0])."<BR>";			
				}
			?>
			</td>            
		</tr>
...
-->
<div style="padding: 0 8px"><button style="width: 100%" class="button3" onClick="javascript:window.location.href='http://www.znck007.com/forum.php?mod=viewthread&tid=27035'" >红外线DIY教程</button></div>
