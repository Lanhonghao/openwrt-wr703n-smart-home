...
if( $_GET['mode']=="getonoff") 
{
	...
	//查找最后一个插座编号
	$sql="SELECT sid,nid,data,note,status,regdate,lasttime FROM api_device WHERE uid='".$uid."' and type='3' ORDER BY sid ASC";
	$result = mysql_query($sql);
	if (!$result)
	{
	  die('Error: ' . mysql_error());
...
<title>智能创客－插座</title>
<meta name="keywords" content="智能创客－插座" />
<meta name="description" content="智能创客－插座" />
...
function onoff(sid,nid){
	...
	if( img.src.indexOf("index44Outlet_")>0 )
	{
		data = "1";
	}else
	{
	...
	if(data == "0"){
		...
		img.src = '../../img/index44Outlet_.png'; //img.src + '?'
	}			
	if(data == "1"){
		//var img = document.getElementById("imgonoff");  //定义一个img对象
		img.src = '../../img/index44Outlet.png'; //img.src + '?'
	}
}
		...
		if(http_request.status == 0 || http_request.status == 200){
			...
			{    
				...
				if(temp.length>2)
				{
					...
					if(temp[2]==0)
					li_html=li_html+"<a onclick=\"onoff('"+temp[0]+"','"+temp[1]+"')\"> <img src=\"../../img/index44Outlet_.png\" id=\"imgonoff"+temp[0]+temp[1]+"\" /></a>";
					if(temp[2]==1)
					li_html=li_html+"<a onclick=\"onoff('"+temp[0]+"','"+temp[1]+"')\"> <img src=\"../../img/index44Outlet.png\" id=\"imgonoff"+temp[0]+temp[1]+"\" /></a>";
					li_html=li_html+"<br />";
			...
			li_html=li_html+"<a href=\"../../devicename.php?mode=adddevice&type=3\" style=\"color:#999999;\"> <img src=\"../../img/index40tool.png\" />";
			li_html=li_html+"<br />";
			li_html=li_html+"添加新插座</a>";					
...
<body onLoad="makeRequest('?mode=getonoff&type=3')">
	...
	<div class="zy">
        <ul id="device">
            ...
            <!--li>
                ...
                <div class="zycz">
                   <a > 
				   <img src="../../img/index44Outlet.png" />                    
					</a>
					<br />
					<a href="kaiguan.php?sid=3&nid=1" style="text-decoration:underline;"> 插座1</a>
				</div>
		   ...
		   <li>				
                <div class="zycz">
					<a href='../../devicename.php?mode=adddevice&type=3' style="color:#999999;"> 
