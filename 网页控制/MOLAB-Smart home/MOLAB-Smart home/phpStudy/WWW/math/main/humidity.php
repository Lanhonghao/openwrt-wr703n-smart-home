...
<title>智能创客－湿度</title>
<meta name="keywords" content="智能创客－湿度" />
<meta name="description" content="智能创客－湿度" />
 	...
 	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<?php 
				...
				$sid=sprintf("%03d",$sid);
				$nid=sprintf("%03d",$nid);		
				//$sql="select floor(time/1000)*1000 as minutes,avg(humidity) from api_temperature WHERE uid='".$uid."' and sid='".$sid."' and nid='".$nid."' group by minutes order by time desc limit 0,30";//5分钟	
				$sql="SELECT time,humidity FROM api_temperature WHERE uid='".$uid."' and sid='".$sid."' and nid='".$nid."' order by time desc limit 0,30";			
				$result = mysql_query($sql);
				if (!$result)
				{
				  die('Error: ' . mysql_error());
				...
				while($row = mysql_fetch_array($result)) 
				{
					...
					$time=$row['time'];
					*/
					echo "湿度：".sprintf("%02d",$row[1])."%时间：".getTimeDifference($row[0])."<BR>";			
				}
			?>
			</td>            
		</tr>
...
<div style="padding: 0 8px"><button style="width: 100%" class="button2" onClick="javascript:window.location.href='?mode=list'" >查看湿度记录</button></div><BR>
