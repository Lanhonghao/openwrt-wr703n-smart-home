...
<title>智能创客－空气质量</title>
<meta name="keywords" content="智能创客－空气质量" />
<meta name="description" content="智能创客－空气质量" />
 	...
 	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<?php 
				...
				$sid=sprintf("%03d",$sid);
				$nid=sprintf("%03d",$nid);				
				$sql="SELECT time,data FROM api_datalist WHERE uid='".$uid."' and sid='".$sid."' and nid='".$nid."' order by time desc limit 0,30";
				$result = mysql_query($sql);
				if (!$result)
				{
				  die('Error: ' . mysql_error());
				...
				while($row = mysql_fetch_array($result)) 
				{
					...
					$time=$row['time'];
					*/
					echo "PM2.5：".$row[1]."/m3时间：".getTimeDifference($row[0])."<BR>";			
				}
			?>
			</td>            
		</tr>
...
<div style="padding: 0 8px"><button style="width: 100%" class="button2" onClick="javascript:window.location.href='?mode=list'" >查看PM2.5记录</button></div><BR>
<div style="padding: 0 8px"><button style="width: 100%" class="button3" onClick="javascript:window.location.href='http://www.znck007.com/forum.php?mod=viewthread&tid=28041'" >PM2.5DIY教程</button></div>
