...
if( $_GET['mode']=="onoff") 
{
	...
	$url="http://".$_SERVER['HTTP_HOST']."/tcp/tcpclient.php?data=".urlencode($tcpdata);echo $url;	
...
<title>智能创客－开窗开门</title>
<meta name="keywords" content="智能创客－开窗开门" />
<meta name="description" content="智能创客－开窗开门" />
...
function onoff(sid,nid){
	...
	if( img.src.indexOf("index45dooron_")>0 )
	{
		data = "1";
	}else
	{
	...
	if(data == "0"){
		//var img = document.getElementById("imgonoff");  //定义一个img对象
		img.src = '../../img/index45dooron_.png'; //img.src + '?'
	}			
	if(data == "1"){
		//var img = document.getElementById("imgonoff");  //定义一个img对象
		img.src = '../../img/index45dooron.png'; //img.src + '?'
	}
}
	...
	if(http_request.readyState == 4){
		if(http_request.status == 0 || http_request.status == 200){
			...
			if(str.length>2){
				...
				if( str[3]=="2" ){
					document.getElementById("device_status").innerHTML="<label style=\"color:#FF0000\">异常-(检查开窗开门/sid和nid)</label>";
				}
...
<table class="top" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        ...
        <td>智能创客－开窗开门</td>
        <td><a href="../door/index.php"><button>列表</button></a></td>
    </tr>
</table>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
		...
		<tr>
            <td width="100%" align="center" >
				   ...
				   <?php 
					...
					if( $data=="0" )
					{
						echo '<a onclick="onoff('.$sid.','.$nid.')"> <img src="../../img/index45dooron_.png" id="imgonoff" /> </a>';						
					}
					else
					{						
						echo '<a onclick="onoff('.$sid.','.$nid.')"> <img src="../../img/index45dooron.png" id="imgonoff" /> </a>';		
					}
                ...
                <label style="color:#00CC00" id="device_status"> 
				   <?php 
					...
					{						
						if( $status==0 ) echo '已经停用'; 
					   	if( $status==1 ) echo '当前正常'; 
					   	if( $status==2 ) echo '<label style="color:#FF0000">异常(检查开窗开门/sid和nid)</label>';
		...
		<tr>
            <td width="100%" align="center" >				
				...
				<a href="../../../devicetime.php?mode=&type=<?php echo $type?>&sid=<?php echo $sid?>&nid=<?php echo $nid?>" style="color:#33CC33;">				                  
				   <?php 				  
...
-->
<div style="padding: 0 8px"><button style="width: 100%" class="button3" onClick="javascript:window.location.href='http://www.znck007.com/forum.php?mod=viewthread&tid=26577'" >开门开窗DIY教程</button></div>
