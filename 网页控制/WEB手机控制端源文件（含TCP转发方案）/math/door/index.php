...
if( $_GET['mode']=="getonoff") 
{
	...
	//查找最后一个电灯开关编号
	$sql="SELECT sid,nid,data,note,status,regdate,lasttime FROM api_device WHERE uid='".$uid."' and type='6' ORDER BY sid ASC";
	$result = mysql_query($sql);
	if (!$result)
	{
	  die('Error: ' . mysql_error());
		...
		if(http_request.status == 0 || http_request.status == 200){
			...
			{    
				...
				if(temp.length>2)
				{
					...
					if(temp[2]==0)
					li_html=li_html+"<a onclick=\"onoff('"+temp[0]+"','"+temp[1]+"')\"> <img src=\"../../img/index45dooron_.png\" id=\"imgonoff"+temp[0]+"\" /></a>";
					else//if(temp[2]==1)
					li_html=li_html+"<a onclick=\"onoff('"+temp[0]+"','"+temp[1]+"')\"> <img src=\"../../img/index45dooron.png\" id=\"imgonoff"+temp[0]+"\" /></a>";
					li_html=li_html+"<br />";
			...
			li_html=li_html+"<a href=\"../../devicename.php?mode=adddevice&type=6\" style=\"color:#999999;\"> <img src=\"../../img/index40tool.png\" />";
			li_html=li_html+"<br />";
			li_html=li_html+"添加新窗门</a>";					
...
<body onLoad="makeRequest('?mode=getonoff&type=4')">
	...
	<div class="zy">
        <ul id="device">
            <!--li>
                <div class="zycz">
                   <a > 
				   <img src="../../img/index45dooron.png" />                    
					</a>
					<br />
					<a href="kaiguan.php?sid=3&nid=1" style="text-decoration:underline;"> 开门开窗1</a>
				</div>
		   ...
		   <li>				
                <div class="zycz">
					<a href='../../devicename.php?mode=adddevice&type=6' style="color:#999999;"> 
