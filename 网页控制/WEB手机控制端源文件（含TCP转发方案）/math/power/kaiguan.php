	...
	if(http_request.readyState == 4){
		if(http_request.status == 0 || http_request.status == 200){
			...
			if(str.length>2){
				...
				if( str[3]=="2" ){
					document.getElementById("device_status").innerHTML="<label style=\"color:#FF0000\">异常-(检查插座/sid和nid)</label>";
				}
...
<table class="top" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        ...
        <td>智能创客－插座</td>
        <td><a href="../power/index.php"><button>列表</button></a></td>
    </tr>
</table>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
		...
		<tr>
            <td width="100%" align="center" >
				   ...
				   <?php 
					...
					if( $data=="0" )
					{
						echo '<a onclick="onoff('.$sid.','.$nid.')"> <img src="../../img/index44Outlet_.png" id="imgonoff" /> </a>';						
					}
					else
					{						
						echo '<a onclick="onoff('.$sid.','.$nid.')"> <img src="../../img/index44Outlet.png" id="imgonoff" /> </a>';		
					}
                ...
                <label style="color:#00CC00" id="device_status"> 
				   <?php 
					...
					{						
						if( $status==0 ) echo '已经停用'; 
					   	if( $status==1 ) echo '当前正常'; 
					   	if( $status==2 ) echo '<label style="color:#FF0000">异常(检查插座/sid和nid)</label>';
		...
		<tr>
            <td width="100%" align="center" >				
				...
				<a href="../../../devicetime.php?mode=&type=3&sid=<?php echo $sid?>&nid=<?php echo $nid?>" style="color:#33CC33;">				                  
				   <?php 				  
		...
		<tr>
			...
			<?php 
				...
				while($row = mysql_fetch_array($result)) 
				{
					...
					$time=$row['time'];
					*/
					echo "插座：".sprintf("%02d",$row[1])."时间：".getTimeDifference($row[0])."<BR>";			
				}
			?>
			</td>            
		</tr>
...
-->
<div style="padding: 0 8px"><button style="width: 100%" class="button3" onClick="javascript:window.location.href='http://www.znck007.com/forum.php?mod=viewthread&tid=26577'" >插座DIY教程</button></div>
